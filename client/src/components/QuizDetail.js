...
function QuizDetail() {
      ...
      if (!quizData) {
        ...
        if (quizId) {
          try {
            ...
            const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/external-quizzes?category=${encodeURIComponent(category)}&amount=10`);
  ...
  const handleSubmit = useCallback(() => {
    ...
    axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/scores`, {
      quizId: quiz._id,
